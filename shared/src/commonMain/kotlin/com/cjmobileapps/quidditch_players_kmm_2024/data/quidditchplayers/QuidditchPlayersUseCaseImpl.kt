...
class QuidditchPlayersUseCaseImpl(
    private val quidditchPlayersRepository: QuidditchPlayersRepository,
) : QuidditchPlayersUseCase {
    private val tag = QuidditchPlayersUseCaseImpl::class.simpleName ?: ""

    private var currentPlayer: PlayerState? = null

    override fun getCurrentPlayer() = currentPlayer

    override fun setCurrentPlayer(player: PlayerState) {
        this.currentPlayer = player
    }

    override suspend fun getHousesFromDB(onHousesResponse: (ResponseWrapper<List<House>>) -> Unit) {
        try {
    ...
    override suspend fun fetchHousesApi(): ResponseWrapper<Boolean> {
    ...
    override suspend fun fetchPlayersAndPositionsApis(houseName: String): ResponseWrapper<Boolean> {
        var responseWrapper: ResponseWrapper<Boolean> = ResponseWrapper()
    ...
    override suspend fun getAllPlayersToDB(onPlayersResponse: (ResponseWrapper<List<PlayerEntity>>) -> Unit) {
        try {
